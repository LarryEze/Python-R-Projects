...
import pandas as pd
with open('datasets/git_log_excerpt.csv', encoding='utf-8') as f:
    read_data = f.read()
    print(read_data)
...
git_log = pd.read_csv('datasets/git_log.gz', sep='#', header=None,
                      encoding='latin-1', names=['timestamp', 'author'])
...
commits_per_year.plot(
    kind='bar', title='commits per year (Linux kernel)', legend=False)
